<?php
    ...
    class Pedidos extends Conectar{
        ...
        public  function get_pedidos(){
            ...
            parent::set_names();
            $sql="SELECT * FROM g5_19.ma_pedidos";
        ...
        public function get_pedido($ID){
            $conectar=parent::conexion();
            parent::set_names();
            $sql="SELECT * FROM g5_19.ma_pedidos WHERE ID = ?";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $ID);
            $sql->execute();
            return $resultado=$sql->fetchAll(PDO::FETCH_ASSOC);
        ...
        public function insert_pedidos($ID,$ID_SOCIO,$FECHA_PEDIDO,$DETALLE,$SUB_TOTAL,$TOTAL_ISV,$TOTAL,$FECHA_ENTREGA,$ESTADO){
            $conectar=parent::conexion();
            parent::set_names();
            $sql="INSERT INTO g5_19.ma_pedidos(ID,ID_SOCIO,FECHA_PEDIDO,DETALLE,SUB_TOTAL,TOTAL_ISV,TOTAL,FECHA_ENTREGA,ESTADO)
            VALUES (?,?,?,?,?,?,?,?,?);";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $ID);
        ...
        Public function update_pedidos($ID,$ID_SOCIO,$FECHA_PEDIDO,$DETALLE,$SUB_TOTAL,$TOTAL_ISV,$TOTAL,$FECHA_ENTREGA,$ESTADO){
            $conectar=parent::conexion();
            parent::set_names();
            $sql="UPDATE g5_19.ma_pedidos SET ID_SOCIO=?,FECHA_PEDIDO=?,DETALLE=?,SUB_TOTAL=?,TOTAL_ISV=?,TOTAL=?,FECHA_ENTREGA=?,ESTADO=?
            WHERE ID=?;";
            $sql=$conectar->prepare($sql);
        ...
        public  function delete_pedidos($ID){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="DELETE FROM g5_19.ma_pedidos  WHERE  ID = ?";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $ID);
            $sql->execute();
            return $resultado=$sql->fetchAll(PDO::FETCH_ASSOC);
